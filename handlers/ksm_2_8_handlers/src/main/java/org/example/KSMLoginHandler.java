 ...
 * and the {at link OAuthBearerValidatorCallbackHandler} is used on the broker to validate the JWT
 ...
 * {at link org.apache.kafka.common.config.SaslConfigs#SASL_OAUTHBEARER_TOKEN_ENDPOINT_URL}
 ...
 *
 * <ul>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_LOGIN_CALLBACK_HANDLER_CLASS}</li>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_LOGIN_CONNECT_TIMEOUT_MS}</li>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_LOGIN_READ_TIMEOUT_MS}</li>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_LOGIN_RETRY_BACKOFF_MS}</li>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_LOGIN_RETRY_BACKOFF_MAX_MS}</li>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_JAAS_CONFIG}</li>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_OAUTHBEARER_TOKEN_ENDPOINT_URL}</li>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_OAUTHBEARER_SCOPE_CLAIM_NAME}</li>
 *   <li>{at link org.apache.kafka.common.config.SaslConfigs#SASL_OAUTHBEARER_SUB_CLAIM_NAME}</li>
 * </ul>
 * </p>
 */

public class KSMLoginHandler implements AuthenticateCallbackHandler {

    private static final Logger log = LoggerFactory.getLogger(KSMLoginHandler.class);
    ...
    public void configure(Map<String, ?> configs, String saslMechanism, List<AppConfigurationEntry> jaasConfigEntries) {
        ...
        log.info("KSM Login handler CONFIGURE");
        moduleOptions = KSMJaasOptionsUtils.getOptions(saslMechanism, jaasConfigEntries);
        this.init();
    }

    public void init() {

        log.info("KSM Login handler INIT");
    ...
    public void close() {
        ...
        log.info("KSM Login handler CLOSE");
    ...
    public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException {
        ...
        log.info("KSM Login handler HANDLE");
    ...
    private void handleTokenCallback(OAuthBearerTokenCallback callback) throws IOException, ExecutionException, InterruptedException {


        log.info("KSM Login handler HANDLETOKENDCALLBACK");

        checkInitialized();
        //String accessToken = accessTokenRetriever.retrieve();

        //System.out.println("KSM HANDLER JAAS CONFIG " + moduleOptions);

        System.out.println(moduleOptions);

        OAuthService oauths = new ServiceBuilder(moduleOptions.get("clientId").toString())
                .apiSecret(moduleOptions.get("clientSecret").toString())
                .build(KeycloakApi.instance(moduleOptions.get("keycloak_instance").toString(), "FER"));
        //System.out.println("KSM Login handler Fetching the Authorization URL...");
        final String authorizationUrl = ((OAuth20Service) oauths).getAuthorizationUrl();

        //System.out.println("KSM Login handler Got the Authorization URL!");
        //System.out.println("KSM Login handler Now go and authorize ScribeJava here:");
        //System.out.println(authorizationUrl);

        final OAuth2AccessToken accessToken = ((OAuth20Service) oauths).getAccessTokenClientCredentialsGrant();
        //System.out.println("KSM Login handler Got the Access Token!");
        //System.out.println("KSM Login handler (The raw response looks like this: " + accessToken.getRawResponse() + "')");
        //System.out.println();
        ...
        //System.out.println("KSM Login handler DECODED: " + decodedString);


        //log.info("KSM Login handler Checkpoint 5 token:  {}", accessToken);
        //System.out.println("KSM Login handler pRINT Checkpoint 5 token: ");
        JSONObject jo = new JSONObject(decodedString);
        //System.out.println(jo.get("alg"));

        KSMBasicOAuthBearerToken mojToken = new KSMBasicOAuthBearerToken(
    ...
    private void handleExtensionsCallback(SaslExtensionsCallback callback) {
        ...
        //log.info("KSM Login handler HANDLEEXTENSIONCALLBACK");
        checkInitialized();
    ...
    private void checkInitialized() {
        //log.info("KSM Login handler CHECKINITIALIZED");
